...
const PhoneNumberQueries = ({ majorRights, baseURL }) => {
  ...
  const handleDeleteAll = async () => {
    const userInput = prompt("Type 'TRUE' to confirm deletion of all Phone Number data:");
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setPhoneData([]); // Clear the state or refetch the blogs
  ...
  const fetchDataAndDownloadExcel = async () => {
    try {
      ...
      saveAs(blob, 'PhoneNumberdata.xlsx'); // Name your file as needed
    } catch (error) {
